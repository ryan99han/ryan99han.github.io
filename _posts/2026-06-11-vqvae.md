...
## Introduction
- VQ-VAEs combine VAEs with discrete latent representations, providing two advantages over standard VAEs
...
$$L_{codebook} = \lVert sg[z_e] - e \rVert ^ 2$$ 
...
$$L_{commitment} = \beta \lVert z_e - sg[e] \rVert ^ 2$$ 

- Thus the final loss is:

$$L = log(p \mid z_q(x)) + \lVert sg[z_e] - e \rVert ^ 2 + \beta \lVert z_e - sg[e] \rVert ^ 2$$ 

## Prior
- Once the encoder has discovered useful discrete codes, a completely separate model can be trained on these
